    ...
    with lzma.open(filename,'rt') as file:
        ...
        for line in file:
            ...
            if epoch_ms:
                ...
                if pmd:
                    # We have a pmd section, find a subsection (datapath, iterations, packets) for this pmd:
                    # 
                    ...
                    # the percent of time, not the percent of iterations.
                    #
                    # The side effect of this is that one cannot diff a stat which is a 'count' from one sample to the next because
                    # the count gets reset to 0 just after every sample. This would typically be done for other tools, getting
...
def dpctl_dump_flows(filename: str):
    ...
    ip_proto_lookup_table = {'0': 'NA', '1': 'ICMP',\
        '2': 'IGMP', '6': 'TCP', '17': 'UDP', '47': 'GRE'}

    # lower case hex string for comparison
    ...
    with lzma.open(filename,'rt') as file:
        for line in file:
            if 'DATE' in line:
                ...
                # check expired flows
                # flow freshness is stamped with curr_stamp when a flow appears in the file
